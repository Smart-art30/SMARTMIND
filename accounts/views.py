...
def get_blog_categories():
    """
    Load all categories and attach their published posts.
    """
    categories = list(
    ...
    for category in categories:
        category.dashboard_posts = posts_by_category.get(
            category.id,
            []
        )
...
def get_blog_context():
    """
    Common blog data available to all dashboards.
    """
    return {
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Account created successfully. Welcome to SmartCoach."
            )
...
def school_admin_dashboard(request):
    ...
    context = {
        ...
        "classes_count": SchoolClass.objects.filter(
            school=school
        ).count(),
        ...
        "recent_assignments": assignments_qs.order_by(
            "-created_at"
        )[:5],

        "recent_attempts": quiz_attempts_qs.order_by(
            "-started_at"
        )[:5],
...
def teacher_create(request):
    ...
    form.fields[
        "teacher_role"
    ].queryset = request.user.school.teacher_roles.all()
...
def teacher_delete(request, pk):
    ...
    messages.success(
        request,
        "Teacher account has been deactivated."
    )
...
def teacher_permissions(request, pk):
    ...
    permissions, _ = TeacherPermission.objects.get_or_create(
        teacher=teacher
    )
